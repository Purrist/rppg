# INSTRUCTION.md - 毕设整体规划

> 本文档包含项目开发、实验迭代、调研、设计文书工作等，是需要时常回顾和更新的文档。
...
1.认知训练/游戏（听音猜歌名、色词测试、打地鼠、处理速度训练等）
2.认知看板（训练时长、训练效果、趋势等等）
3.学习（园艺种植、烹饪烘焙、琴棋书画、手工（织毛衣、刺绣、剪纸、折纸、串珠））

##### 认知训练游戏设计原则（2024-03-19 更新）

**四状态流程**：所有认知训练游戏必须遵循统一的四状态流程：

1. **待机状态 (IDLE)**：显示游戏列表，等待用户选择
2. **预备状态 (READY)**：显示游戏规则，等待用户站入区域
3. **游戏中 (PLAYING)**：执行游戏逻辑，显示控制按钮
4. **结算状态 (SETTLING)**：显示成绩，提供重玩/退出选项

**游戏组件化**：每个游戏必须有独立的Vue组件，便于扩展和维护。

**已实现的游戏**：
- 打地鼠 (whack_a_mole)：锻炼反应速度
- 处理速度训练 (processing_speed)：基于ACTIVE研究，包含Go/No-Go、选择反应、序列反应三个模块
...
### 6.2 自适应认知陪伴智能体（Adaptive Cognitive Companion Agent）
...
- 视频/音频/戏曲/广场舞：**正在看什么、看了多久、反复看什么**
- 操作行为：点哪里、停在哪里、有没有犹豫
- 面部表情：DeepFace → 开心/无聊/疲劳/困惑
- 生理状态（可扩展）：心率、睡眠、坐姿

**输出：用户状态向量**
```json
{
  "current_content": "西游记·三打白骨精",
  "attention": 0.85,
  "emotion": "happy",
  "fatigue": 0.2,
  "last_action": "repeat_play"
}
```

#### 二、理解层（它的大脑：真正理解世界）

这里不用规则，不用人工配置，**全部由大模型本地实时理解。**
...
```json
{
  ...
  "relationships": [...],
  "scenes": [...],
  "training_type": ["记忆", "识别", "联想"]
}
```

#### 三、思考层（它的意识：自主决策）

这是**活系统的灵魂**，它会**自己决定现在该做什么**。
...
它能调用的工具：
- `search_image(keyword)` → 自动搜图
- `search_audio(keyword)` → 自动搜音频
- `search_video(keyword)` → 自动搜片段
- `generate_game(...)` → 生成结构化题目
- `speak(text)` → 语音鼓励
- `adjust_difficulty()` → 自动调整难度

你前端只需要：**接收最终JSON → 渲染界面。**
...
不需要 OpenClaw，**只用 Qwen3.5本地 + 工具调用 = 活系统。**
...
### 一、认知训练
1.认知训练对老年人是否真的有用？
2.怎么通过实验/可用性测试等检测认知训练有用？
3.有哪些老年人的认知训练方法被认证是有效的？
4.这些有用的方法通过什么机制发挥作用？
5.这些训练方法是否有明确的定义和设计方法？
6.哪些因素影响认知训练效果？
