...
def get_game_state():
    print(f"[DEBUG] gamedemo.py: 返回游戏状态: {game_state}")
...
def update_game_state():
    ...
    global game_state
    data = request.json
    print(f"[DEBUG] gamedemo.py: 收到更新数据: {data}")
    
    if data:
        if 'difficulty' in data:
            new_diff = max(1, min(8, int(data['difficulty'])))
            game_state['difficulty'] = new_diff
            game_state['difficultyName'] = DIFFICULTY_NAMES.get(new_diff, '简单')
            print(f"[DDA] 难度已更新: {new_diff} - {game_state['difficultyName']}")
            # 删除 difficulty 字段，让其他字段可以被更新
            data.pop('difficulty', None)
        
        game_state.update(data)
        print(f"[DEBUG] gamedemo.py: 更新后状态: {game_state}")
