# 结合健康监测与认知训练的老年陪伴机器人交互体验设计研究（开发文档）
...
---
### 2. 我向ai提出过的要求（从过去到现在），当有矛盾的点时，以最新的要求为准
...
19
硬件适配：完成了从网络视频流到本地 USB 摄像头的底层兼容性切换。通过 isdigit() 逻辑实现了 sys.argv 字符串到 int 索引的自动转换。
性能优化：维持 ScreenProcessor 内的独立采集线程（_worker），确保本地硬件开启后，判定逻辑依然保持零延迟响应。

20
投影交互系统重构：
- 问题解决：外接摄像头左右镜像问题，通过 cv2.flip(frame, 1) 实现水平翻转
- 透视校正：实现四顶点手动校准 + 透视变换（Perspective Transform），将梯形投影区域校正为矩形
- 检测方案：从 MediaPipe Hands 改为背景减法（MOG2）检测脚部，更适合地面投影场景
- 校准流程：首次启动弹出 Calibration 窗口，按左上→右上→右下→左下点击投影四个角完成校准
- 交互优化：脚踩停留时间从 2 秒调整为 1.5 秒，提升响应速度
- 调试显示：开发者页面实时显示透视校正后的画面和运动检测掩码

21
脚部检测准确性提升与游戏参数优化：
- 检测方案重构：完全弃用背景减法，改用 MediaPipe Pose 检测完整人体骨架
- 脚部定位逻辑：检测到完整人体后，选择左右踝/脚尖中 y 坐标最大的点作为脚部位置（最下方的脚）
- 摄像头物理旋转处理：新增 rotate_frame 函数，先旋转90度再水平翻转
- 地鼠停留时间：调整为 4秒，给足反应时间
- 脚踩判定时间：调整为 2秒，进度圈从 0 到 100% 完整加载
- 分数逻辑：正确 +10分，错误 -5分
- 进度圈配色：加载中为灰蓝色(#6496C8)，正确为安全绿(#33B555)，错误为警戒红(#FB4422)
- 检测调试：开发者页面显示完整人体骨架连线，绿色大圆圈标记检测到的脚部位置
