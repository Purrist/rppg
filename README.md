...
智能感知与多模态交互体验系统 - 开发文档
1. 项目愿景与定义
本项目开发一套集成“非接触式信号采集 + 认知辅助训练 + 智能助理交互”的综合软件原型。

比例标准：强制 16:10 适配（参考分辨率 2560*1600），确保 UI 元素在目标终端上不拉伸、不溢出。

核心技术：

rPPG 引擎：利用摄像头捕捉皮肤表层微弱的绿色通道色彩波动（Chroma 算法），转化为波形数据。

状态识别：基于面部关键点与深度学习模型，判定当前使用者的情感倾向（平静、愉悦、压力等）。

空间映射：通过外接设备识别肢体进入虚拟交互区的行为。
...
锁定 Forehead(额头) 与 Cheeks(两颊) 三个 ROI 区域。

采样频率固定为 30fps，用于提取信号序列。

外接端 (Processor B)：

采用目标遮挡逻辑，将物理空间的动作映射为虚拟按键指令。

2.2 数据枢纽 (通信层)
采用 eventlet 高并发异步模式。

信号流与视频帧同步打包推送，确保感知结果与画面无延迟对应。

3. 开发日志与进度记录
v1.2 - 核心引擎驱动 (2026-01-22)
【实现内容】

真实 rPPG 采样：实现了基于色彩分量的波形提取，通过 Butterworth 带通滤波器去除高频噪声。

多页面骨架实装：一次性搭建了首页、健康、娱乐、学习、个人、开发者、训练 7 大核心页面。

交互组件：完成智能助理 Akon 的“悬浮球-面板”二级切换逻辑。

16:10 容器锁：前端根组件 app.vue 实现了强制比例显示。

【已解决问题】

[BUG修复]：修复了 socket.io-client 缺失导致的 Vite 编译错误。

[BUG修复]：修复了 WinError 6 句柄无效错误，通过锁定后端端口至 5000 并关闭 Flask 的热重载解决。

【待处理问题】

算法精度：目前 rPPG 的信号处理对光照依然敏感，后续需增加肤色归一化逻辑。

v1.3 - 交互架构大重组 (2026-01-22)
【实现内容】

全局导航实装：侧边栏从页面内移至 app.vue 顶层，实现跨页面常驻。

二级账户体系：实现了侧边栏底部个人信息点击展开“设置/登录/开发者”的功能面板。

交互优化：

Akon 面板增加全屏遮罩（Mask），点击外部自动收起。

Akon 面板增加大尺寸关闭按钮 [X]。

非首页增加全局“返回”按钮。

页面补全：实装了设置页（Settings）的基础布局。