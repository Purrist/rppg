...
传统游戏 DDA 核心是提高沉浸感与竞技体验，例如 FPS、赛车、MOBA，主要优化挑战性、刺激感、胜负平衡，常见输入 score、rate、kill/death、reaction speed 即可完成难度调整。代表性研究 Robin Hunicke 提出的 DDA 框架指出，玩家表现（performance）通常可用于实时调整游戏挑战水平。但认知训练不是竞技游戏。
...
同一位老人，上午 difficulty = 5 可以顺利完成，但疲劳后同样 difficulty = 5 可能已经明显过难，即能力是动态变化的。

### （2）情绪对认知影响更强

老年群体更容易挫败、焦虑、放弃。研究发现，消极情绪会显著降低工作记忆与持续参与意愿。因此正确率高 ≠ 体验好。

### （3）存在安全与疲劳风险

你已经有 HR、HRR、HR slope，这意味着系统不仅需要调难度，还需要判断老人累不累、是否超负荷，这在 serious game 和 digital therapeutics 中非常重要。

因此你的目标应该是构建“状态驱动型难度系统”(State-driven Difficulty Adjustment)，而不是表现驱动型系统(Performance-driven DDA)。

# 2. 为什么传统 DDA 不够

传统 DDA 通常是高正确率 → 升难度，低正确率 → 降难度，例如：
```python
if accuracy > 0.8:
    difficulty += 1
if accuracy < 0.5:
...
例如老人正确率 = 85%，看起来应该升难度，但实际上 HRR 高、HR持续上升、消极高信度、RT变慢，说明他其实在硬撑。此时升难度会直接导致挫败 → 放弃训练，这是老年训练最危险的问题。

## 2.2 无法识别“无聊”

另一种情况是正确率 90%、RT越来越快、HRR极低、情绪中性，说明太简单了，用户实际上 disengaged（失去投入），即会“挂机式完成”，长期训练效果下降。

## 2.3 无法处理状态冲突

例如正确率高但情绪差，或者正确率低但很投入，单指标无法解释这种多模态冲突问题。

因此近年来 adaptive system 研究开始转向 Multimodal User State Estimation（多模态用户状态估计）。研究共识是 performance 是能力指标，physiology + affect 是内部状态指标，不能混为一谈。相关研究 Affective Computing 以及《User Experience With Dynamic Difficulty Adjustment Methods for an Affective Exergame》发现，physiology 可以提升 adaptation accuracy，但不保证直接提升体验，说明生理数据不能直接决定难度，而应用于状态推断。

# 3. 为什么不推荐直接用 Scikit-Fuzzy

你想到 fuzzy 其实很合理，因为你的变量全是模糊状态：累不累、开心不开心、投入不投入、能不能继续，天然像 fuzzy logic。但对于你的毕设，第一版不建议。

## 3.1 Fuzzy 最大问题：解释性不足

例如系统输出 difficulty = 0.43，老师会问为什么是 0.43？你很难解释，因为 membership function 通常人工划定，例如 medium HRR 的边界怎么定，很主观，容易被问依据是什么。

## 3.2 Fuzzy 参数很难写论文

例如 IF HRR medium AND emotion slightly negative THEN difficulty = small decrease，老师会追问为什么不是大 decrease？很难给权威依据。

## 3.3 你现在没有足够训练数据

Fuzzy 更适合大量历史数据 + 专家规则优化，你现在没有 100+ 老人 longitudinal data，因此容易过拟合。

## 3.4 Fuzzy 不擅长处理信号可靠性

你现在存在 hr abnormal 时 hrr = 0、hrslope = 0，这意味着数据可靠性变化，Fuzzy 对 signal reliability 处理不好，容易误判。

因此相比 fuzzy 更推荐 Hierarchical State Estimation 和 Evidence Fusion（下一部分开始正式讲这个是什么，以及为什么是当前更前沿的方法）。

# Part 2 / Hierarchical State Estimation（分层状态估计）是什么，为什么适合你
# 4. 什么是 Hierarchical State Estimation？
...
下面是你这个系统的**完整状态机（Complete Hierarchical State Machine）**。我按：
...
└──────────────────────────────────────────┘
                     ...
                     ▼
┌──────────────────────────────────────────┐
│        Reliability Gate Layer            │
│──────────────────────────────────────────│
│ HR abnormal ?                            │
│                                          │
│ YES → Physiology Weight = 0              │
│ NO  → Full Evidence Fusion               │
└──────────────────────────────────────────┘
                     │
                     ▼
┌────────────────────┬─────────────────────┐
...
│────────────────────│─────────────────────│
│ Ability Estimation │ Engagement Estimate │
│                    │                     │
│ Strong             │ Highly Engaged      │
│ Capable            │ Engaged             │
│ Struggling         │ Passive             │
│ Overloaded         │ Withdrawal Risk     │
└────────────────────┴─────────────────────┘
                     │
                     ▼
┌──────────────────────────────────────────┐
...
```text
           ...
           YES                   NO
            ...
            ▼                     ▼
┌────────────────────┐   ┌────────────────────┐
│ Physiology Enabled │   │ Physiology Disabled│
│                    │   │                    │
│ HRR active         │   │ HRR ignored        │
│ HR slope active    │   │ HR slope ignored   │
└────────────────────┘   └────────────────────┘
```
...
```text
 ...
 HR abnormal   Sustained High HRR  Collapse

     │              │              │
     ▼              ▼              ▼

Disable HR     Force -1           Force -1
