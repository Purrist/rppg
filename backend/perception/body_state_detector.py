...
class BodyStateDetector:
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, Any]:
        if frame is None or frame.size == 0: return self._get_default_result()
        ...
        try:
            ...
            self.pose_history.append({"posture": result["posture"], "activity_level": result["activity_level"], "time": time.time()})
    ...
    def _get_default_result(self) -> Dict[str, Any]:
        return {"posture": self.current_posture, "activity_level": self.activity_level, "activity_duration": time.time() - self.posture_start_time, "head_pose": "frontal", "movement_frequency": self.movement_frequency, "stillness_duration": time.time() - self.stillness_start_time}
    ...
    def _detect_with_mediapipe(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        if pose_result.pose_landmarks:
            ...
            self.position_history.append({"x": shoulder_center_x, "y": shoulder_center_y, "time": current_time})
            ...
            if shoulder_hip_dist > 0 and hip_knee_dist > 0:
                if hip_knee_dist < shoulder_hip_dist * 0.5: result["posture"] = "sitting"
                else: result["posture"] = "standing"
            if abs(shoulder_center_y - hip_center_y) < 0.1: result["posture"] = "lying"
            
            if len(self.position_history) >= 30:
                recent = list(self.position_history)[-30:]
                positions = [(p["x"], p["y"]) for p in recent]
                total_movement = sum(np.sqrt((positions[i][0]-positions[i-1][0])**2 + (positions[i][1]-positions[i-1][1])**2) for i in range(1, len(positions)))
                if total_movement > 1.0 and result["posture"] == "standing": result["posture"] = "walking"
            
            ear_diff_x = left_ear.x - right_ear.x
            if abs(ear_diff_x) > 0.1: result["head_pose"] = "right" if ear_diff_x > 0 else "left"
            elif nose.y < shoulder_center_y - 0.3: result["head_pose"] = "up"
            elif nose.y > shoulder_center_y - 0.1: result["head_pose"] = "down"
            else: result["head_pose"] = "frontal"
        return result
